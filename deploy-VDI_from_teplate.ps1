...
$template = "VDI_template"

$a = Get-Datastore V3700_VDI_4
...
$osCust = Get-OSCustomizationSpec -name "VDI template"
$resPool = Get-ResourcePool -Name "Other"

New-VM -name $vmName -ResourcePool $resPool -Location $folder -Datastore $a -Template $template -OSCustomizationSpec $osCust
...
get-adcomputer $vmName | Move-ADObject -targetPath "OU=VDI,OU=Windows 10,OU=Desktops,OU=Computers,OU=UK Crawley,DC=intranet,DC=macro4,DC=com"
