...
# Reference: -
# Description: Check ESXi 6.5 support with JSON based HCL provided by www.virten.net
...
foreach ($esx in $check){
  ...
  if($esx.SupportedReleases){
    if ($esx.SupportedReleases -match "6.7"){
      Write-Host "ESXi 6.7 supported" -ForegroundColor Green
    } else {
      Write-Host "ESXi 6.7 unsupported" -ForegroundColor Red
    }
  } else {
    Write-Host "unknown"
  }
