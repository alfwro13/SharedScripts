...
# #############################################
#
# Here you specify where your data is saved 
#

$SharedFolder = "\\M4TC240\McAfee\test"

###############################################

$strFileName=$SharedFolder
If (Test-Path $strFileName){
}Else{
  "Specified location " + $SharedFolder + " does not exist or unreachable"
  Break  
}

$hostname = [System.Net.Dns]::GetHostName()
$FullFilePathHost = "$SharedFolder" + "\" + "$hostname" + ".csv"

# Fail script if we can't find SecEdit.exe
# SecEdit should be available on all Windows systems (key word is should)
...
if ( $LASTEXITCODE -eq 0 ) {
  $output = Select-String '^(Se\S+) = (\S+)' $TemplateFilename | Foreach-Object {
...
$output | Where-Object {$_.Privilege -eq 'SeInteractiveLogonRight' } | Export-Csv -NoTypeInformation -Path $FullFilePathHost
write-host "Output saved to:" $FullFilePathHost